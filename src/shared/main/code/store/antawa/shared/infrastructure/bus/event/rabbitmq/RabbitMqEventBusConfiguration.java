...
import store.antawa.shared.infrastructure.bus.event.DomainEventSubscribersInformation;
...
public class RabbitMqEventBusConfiguration {
    ...
    private Collection<Collection<Declarable>> declareQueuesAndBindings(
        ...
        return domainEventSubscribersInformation.all().stream().map(information -> {
            String queueName           = store.antawa.shared.infrastructure.bus.event.rabbitmq.RabbitMqQueueNameFormatter.format(information);
            String retryQueueName      = store.antawa.shared.infrastructure.bus.event.rabbitmq.RabbitMqQueueNameFormatter.formatRetry(information);
            String deadLetterQueueName = store.antawa.shared.infrastructure.bus.event.rabbitmq.RabbitMqQueueNameFormatter.formatDeadLetter(information);
